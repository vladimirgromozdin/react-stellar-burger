...
function OrderDetails() {
    const orderId = useSelector((store: any) => store.orderDetails.orderId)
    ...
    if (loading) {
        return (<div className={`${styles.content} pt-10 pr-10 pl-10 pb-4`}>
            <h4 className="text text_type_main-medium pt-30 pb-3">Генерируем идентификатор заказа,</h4>
            <p className="text text_type_main-medium pb-15">нужно ещё несколько секунд...</p>
            <img src={loaderIconPath} alt="order-preloader-icon" className="pb-15"/>
        </div>)
